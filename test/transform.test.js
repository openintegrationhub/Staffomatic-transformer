...
const { userFromOih, userToOih } = require("./seed/person");
const transformUserFromOih = require("../lib/actions/transformUserFromOih");
const transformUserToOih = require("../lib/actions/transformUserToOih");

describe("Transformation test", () => {
  it("should handle simple person tranformation in direction from OIH", () => {
    const exp = userFromOih();
    return transformUserFromOih.process(newMessage(exp)).then((result) => {
      ...
      expect(result.body.data.addresses[0].district).to.be.equal(
        "Alstadt-Sued"
      );
      ...
      expect(result.body.data.contactData[0].description).to.be.equal(
        "primary"
      );
    });
  });
  ...
  it("should produce an empty message if transformation returns undefined", () => {
    return transformUserFromOih.process(newMessage({})).then((result) => {
  ...
  it("should handle simple person tranformation in direction to OIH", () => {
    const exp = userToOih();
    return transformUserToOih.process(newMessage(exp)).then((result) => {
      expect(result.body).to.be.an("object");
      ...
      expect(result.body.data.contactData[1].value).to.be.equal(
        "m.smith@mail.com"
      );
    });
  });
  ...
  it("should produce an empty message if transformation returns undefined", () => {
    return transformUserToOih.process(newMessage({})).then((result) => {
      expect(result).to.be.undefined;
    });
  });
});
